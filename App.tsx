...
const localApi = {
  ...
  getFeed: async (token?: string | null) => {
    return { 
      data: INITIAL_POSTS.map(post => ({
        ...post,
  ...
  getStories: async (token?: string | null) => {
    return { data: INITIAL_STORIES, success: true };
  },
  
  getReels: async (token?: string | null) => {
    return { data: INITIAL_REELS, success: true };
  },
  ...
  reactToPost: async (token: string, postId: number, type: ReactionType) => {
    return { success: true };
  },
  
  createPost: async (token: string, payload: any) => {
...
export default function App({ initialData, initialPath }: { initialData?: any, initialPath?: string }) {
  ...
  // Initial data loading - uses local data only
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        // Load all data from local constants
        const [feedResponse, storiesResponse, reelsResponse, productsResponse] = await Promise.all([
          localApi.getFeed(),
          localApi.getStories(),
          localApi.getReels(),
        ...
        const currentUserData = await localApi.getCurrentUser();
        setCurrentUser(currentUserData);
        
        console.log('Initial data loaded successfully from local constants');
  ...
  useEffect(() => {
    if (isClient) {
      ...
      // Only use localStorage if initial data failed
  ...
  const handleCreatePost = async (
    ...
    // Simulate API call
    try {
      await localApi.createPost('dummy-token', { content: text });
  ...
  const handleReact = (itemId: number, type: ReactionType) => {
    ...
    // Simulate API call
    localApi.reactToPost('dummy-token', itemId, type);
  };
  ...
  // Stub functions for missing brand-related handlers

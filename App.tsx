...
const API_BASE_URL = IS_DEVELOPMENT 
  ...
  : 'https://unera.social';   // Production API
  ...
  switch(endpoint) {
    case '/users/me':
      return {
        ...
        coverImage: 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80',
    ...
    default:
      if (endpoint.startsWith('/users/')) {
        ...
        const user = INITIAL_USERS.find(u => u.id.toString() === userId) || getDefaultUser();
        return user;
      }
      if (endpoint.startsWith('/posts/')) {
        const postId = endpoint.split('/')[2];
        const post = INITIAL_POSTS.find(p => p.id.toString() === postId);
        if (post) {
          return { ...post, formattedTime: formatRelativeTime(post.timestamp || Date.now()) };
        }
      }
...
const apiRequest = async <T = any>(
  ...
  // Development mode: Use mock data if API fails or is not available
  if (IS_DEVELOPMENT) {
    try {
      // Try real API first
    ...
    return await getMockData(endpoint) as T;
  }
  ...
  try {
    ...
    // Even in production, fall back to mock data for better UX
    if (endpoint === '/users/me' || endpoint === '/posts') {
      return await getMockData(endpoint) as T;
    }
...
const getAuthorForPost = (post: PostType, users: User[], brands: Brand[]) => {
    if (post.brandId) {
        ...
        if (brand) {
            return {
                ...
                type: 'brand' as const,
                name: brand.name,
                profileImage: brand.profileImage || '/default-brand.png',
    ...
    if (brandByAuthorId) {
        return {
            ...
            profileImage: brandByAuthorId.profileImage || '/default-brand.png',
...
function App({ initialData, initialPath }: { initialData?: any, initialPath?: string }) {
    ...
    const [users, setUsers] = useState<User[]>(() => {
        const initialUsers = initialData?.users || INITIAL_USERS;
    ...
    const [brands, setBrands] = useState<Brand[]>(() => {
        return INITIAL_BRANDS.map(brand => ({
            ...brand,
            profileImage: brand.profileImage || '/default-brand.png',
            coverImage: brand.coverImage || '/default-cover.jpg'
        }));
    });
    ...
    // Debug logging
    useEffect(() => {
        if (isClient && IS_DEVELOPMENT) {
            console.log('App State Debug:', {
                currentUser: currentUser ? { ...currentUser, profileImage: getSafeProfileImage(currentUser) } : null,
    ...
    }, [currentUser, users.length, posts.length, view, activeTab, apiError, isClient]);
                ...
                try {
                    ...
                    if (token) {
                        ...
                        try {
                            ...
                            console.warn('Failed to load stories:', storiesError);
                        }
                        ...
                        try {
                            ...
                            console.warn('Failed to load music:', musicError);
                        }
                    }
                    
                } catch (error) {
                    console.error('Failed to load data from API:', error);
                    setApiError('Failed to load data. Using offline mode.');
                } finally {
                    setIsLoading(false);
                }
            } else {
        ...
        try {
            ...
            if (data.token && data.user) {
                ...
                try {
                    ...
                    console.warn('Failed to load posts after login:', postsError);
                }
    ...
    const handleCreateProduct = (productData: Partial<Product>) => {
        ...
        const newProduct: Product = {
            ...
            sellerAvatar: getSafeProfileImage(currentUser),
            status: 'active',
            views: 0,
            ratings: [],
            comments: [],
            ...
            if (storedUser) {
                try {
                    ...
                    if (freshUser) {
                        setCurrentUser({
                            ...freshUser,
                            profileImage: freshUser.profileImage || '/default-profile.png'
                        });
                    }
                } catch (e) {
    ...
    const renderContent = () => {
        ...
        if (effectiveView === 'login') {
            if (showRegister) {
                return <Register onRegister={handleRegister} onBackToLogin={() => { setShowRegister(false); setShowForgotPassword(false); }} />;
            } else if (showForgotPassword) {
                return <ForgotPassword onBackToLogin={() => { setShowForgotPassword(false); setShowRegister(false); }} />;
            } else {
                return <Login 
                    onLogin={handleLogin} 
                    onNavigateToRegister={() => { setShowRegister(true); setShowForgotPassword(false); }} 
                    onNavigateToForgotPassword={() => { setShowForgotPassword(true); setShowRegister(false); }} 
                    onClose={() => { setView('home'); setCurrentUser(null); }} 
                    error={loginError} 
                />;
            }
        }
            ...
            <>
                ...
                <div className="flex justify-center w-full max-w-[1920px] mx-auto relative flex-1">
                    <div className="sticky top-14 h-[calc(100vh-56px)] z-20 hidden lg:block">
                        <Sidebar 
                            currentUser={currentUser || { ...getDefaultUser(), profileImage: '/default-profile.png' }} 
                            onProfileClick={(id) => { setSelectedUserId(id); setView('profile'); }} 
                            onReelsClick={() => handleNavigate('reels')} 
                            onMarketplaceClick={() => handleNavigate('marketplace')} 
                            onGroupsClick={() => handleNavigate('groups')} 
                    ...
                    <div className="w-full lg:w-[740px] xl:w-[700px] min-h-screen">
                        ...
                        {effectiveView === 'profile' && selectedUserId !== null && (
                            <UserProfile 
                                user={users.find(u => u.id === selectedUserId) || getDefaultUser()} 
                                currentUser={currentUser} 
                                users={users} 
                                posts={(() => {
                                ...
                                onEditPost={() => {}} 
                                getCommentAuthor={(id) => users.find(u => u.id === id) || getDefaultUser()} 
                ...
                {activeStory && (
                    <StoryViewer 
                        story={activeStory} 
                        user={users.find(u => u.id === activeStory.userId) || getDefaultUser()} 
                        currentUser={currentUser} 
...
// Export the wrapped App component

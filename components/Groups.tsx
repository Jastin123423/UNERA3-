                ...
                <div className="p-4 space-y-4">
                    ...
                    {isAdmin && (
                        <div className="border-t border-red-500/20 pt-4 mt-4">
                            <button onClick={onDeleteGroup} className="w-full bg-red-500/10 text-red-500 font-bold py-2.5 rounded-lg transition-all hover:bg-red-500 hover:text-white border border-red-500/20">
                                Delete Community
                            </button>
                        </div>
                    )}
...
export const GroupsPage: React.FC<GroupsPageProps> = ({ currentUser, groups, users, onCreateGroup, onJoinGroup, onLeaveGroup, onDeleteGroup, onUpdateGroupImage, onPostToGroup, onCreateGroupEvent, onInviteToGroup, onProfileClick, onLikePost, onOpenComments, onSharePost, onDeleteGroupPost, onRemoveMember, onUpdateGroupSettings, initialGroupId, onPlayAudioTrack }) => {
    ...
    // FIX 1: Better initialization - check initialGroupId first
    useEffect(() => { 
        console.log("GroupsPage mounted with:", { initialGroupId, groupsCount: groups.length });
        if (initialGroupId) { 
            console.log("Looking for group with ID:", initialGroupId);
            const group = groups.find(g => g.id === initialGroupId); 
            if (group) { 
                console.log("Found group, switching to detail view:", group.name);
                setActiveGroupId(group.id); 
                setView('detail'); 
                setGroupTab('Discussion'); 
            } else {
                console.error("Group not found with ID:", initialGroupId);
                setView('feed'); // Fallback to feed if group not found
            }
        } else {
            setView('feed'); // Default to feed view
        }
    ...
    // FIX 2: Corrected conditional rendering logic
    // Only show feed if view is 'feed' AND we're not loading a specific group
    if (view === 'feed') {
                            ...
                            <div key={group.id} className="bg-[#242526] rounded-xl overflow-hidden border border-[#3E4042] flex flex-col">
                                ...
                                <div className="p-4 flex-1 flex flex-col">
                                    ...
                                    <div className="mt-auto">
                                        ...
                                        <button onClick={() => currentUser ? onJoinGroup(group.id) : alert("Login first")} className="w-full bg-[#1877F2] hover:bg-[#166FE5] text-white py-2 rounded-lg font-semibold transition-colors">
                                            Join Group
                                        </button>
                                    </div>
                                </div>
    ...
    // FIX 3: Only render detail view if we have an activeGroup
    if (view === 'detail' && !activeGroup) {
        console.log("No active group found, falling back to feed");
        return (
            <div className="w-full max-w-[1000px] mx-auto p-4 font-sans">
                <div className="text-white text-center py-10">
                    <div className="text-2xl mb-4">Group not found</div>
                    <button 
    ...
    // Now we know we have an activeGroup and view is 'detail'
    const isMember = currentUser ? (activeGroup!.members.includes(currentUser.id) || activeGroup!.adminId === currentUser.id) : false;
    const isGroupAdmin = currentUser && activeGroup!.adminId === currentUser.id;
    const canManage = isGroupAdmin || isAdmin;
    const canPost = canManage || (activeGroup!.memberPostingAllowed ?? true);

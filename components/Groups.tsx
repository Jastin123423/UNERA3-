    ...
    return (
        <div className="w-full bg-[#18191A] min-h-screen pb-10">
            ...
            <div className="max-w-[700px] mx-auto px-0 md:px-4">
                ...
                {groupTab === 'About' && (<div className="bg-[#242526] rounded-xl p-8 border border-[#3E4042] mx-4 md:mx-0 shadow-sm animate-fade-in"><h3 className="text-xl font-bold text-[#E4E6EB] mb-4">About this group</h3><p className="text-[#E4E6EB] text-base mb-8 leading-relaxed">{activeGroup.description}</p><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="flex items-center gap-4 text-[#E4E6EB]"><div className="w-12 h-12 bg-[#3A3B3C] rounded-xl flex items-center justify-center"><i className={`fas ${activeGroup.type === 'public' ? 'fa-globe-americas' : 'fa-lock'} text-xl text-[#B0B3B8]`}></i></div><div><div className="font-bold">{activeGroup.type === 'public' ? 'Public' : 'Private'}</div><div className="text-xs text-[#B0B3B8]">Anyone can see who's in the group and what they post.</div></div></div><div className="flex items-center gap-4 text-[#E4E6EB]"><div className="w-12 h-12 bg-[#3A3B3C] rounded-xl flex items-center justify-center"><i className="fas fa-history text-xl text-[#B0B3B8]"></i></div><div><div className="font-bold">History</div><div className="text-xs text-[#B0B3B8]">Created on {new Date(activeGroup.createdDate).toLocaleDateString()}</div></div></div></div></div>)}
            ...
            {showSettingsModal && activeGroup && <GroupSettingsModal group={activeGroup} onClose={() => setShowSettingsModal(false)} onUpdate={(settings) => onUpdateGroupSettings(activeGroup.id, settings)} isAdmin={isAdmin} onDeleteGroup={() => { onDeleteGroup(activeGroup.id); setShowSettingsModal(false); setView('feed'); }} />}

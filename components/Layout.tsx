    ...
    return (
        <div className="w-[300px] h-full overflow-y-auto px-2 pt-4 bg-[#18191A] hidden lg:block scrollbar-hide">
            <div className="flex items-center gap-3 p-2 hover:bg-[#3A3B3C] rounded-lg cursor-pointer transition-colors mb-2" onClick={() => onProfileClick(currentUser.id)}>
                <img src={currentUser.profileImage} alt="" className="w-9 h-9 rounded-full object-cover" />
...
export const RightSidebar: React.FC<RightSidebarProps> = ({ contacts, onProfileClick }) => {
    return (
        <div className="w-[280px] h-full overflow-y-auto pt-4 pr-2 bg-[#18191A] hidden xl:block scrollbar-hide">
            <div className="flex items-center justify-between px-2 mb-2">
                ...
                <div className="flex gap-4 text-[#B0B3B8]">
                    <i className="fas fa-video cursor-pointer hover:bg-[#3A3B3C] p-2 rounded-full transition-colors text-sm"></i>
                    <i className="fas fa-search cursor-pointer hover:bg-[#3A3B3C] p-2 rounded-full transition-colors text-sm"></i>
                    <i className="fas fa-ellipsis-h cursor-pointer hover:bg-[#3A3B3C] p-2 rounded-full transition-colors text-sm"></i>
                </div>
            </div>
            <div className="space-y-1">
                {contacts.map(user => (
                    <div 
                        key={user.id} 
                        className="flex items-center gap-3 p-2 hover:bg-[#3A3B3C] rounded-lg cursor-pointer transition-colors relative group"
                        onClick={() => onProfileClick(user.id)}
                    >
                        <div className="relative">
                            <img src={user.profileImage} alt="" className="w-9 h-9 rounded-full object-cover border border-[#3E4042]" />
                            {user.isOnline && (
            ...
            {contacts.length === 0 && (
                <p className="text-center text-[#B0B3B8] text-sm py-4">No active contacts</p>
            )}
        </div>
    );
};

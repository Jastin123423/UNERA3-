...
const BrandsPage: React.FC<BrandsPageProps> = ({ 
    ...
    onUpdateBrand, onMessage, onCreateEvent, initialBrandId, onPlayAudioTrack
}) => {
    ...
    const brandPosts = useMemo(() => {
        ...
        return posts
            ...
            .map(post => ({
                ...post,
                // Ensure formattedTime exists, calculate if not
                formattedTime: post.formattedTime || formatBrandPostTime(post.timestamp || post.createdAt || Date.now())
            }));
    }, [posts, activeBrand]);
    ...
    // Updated to handle multiple images and files array
                        ...
                        <div className="bg-[#242526] rounded-xl p-4 border border-[#3E4042] mx-4 md:mx-0">
                            ...
                            <div className="grid grid-cols-3 gap-1">
                                {brandPosts
                                    ...
                                    .flatMap(p => {
                                        // Handle both single image (p.image) and multiple images (p.images)
...
// PROFESSIONAL EXPORT STATEMENT - This fixes the deployment error
export { BrandsPage };

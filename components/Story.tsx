            ...
            <div className="relative w-full max-w-[420px] h-full sm:h-[92vh] bg-black sm:rounded-2xl overflow-hidden flex flex-col shadow-2xl">
                ...
                <div className="absolute top-4 left-0 right-0 p-4 z-30 flex items-center justify-between mt-2">
                    <div className="flex items-center gap-3">
                        <img src={user.profileImage} alt={user.name} className="w-12 h-12 rounded-full border-2 border-[#1877F2] object-cover shadow-lg" />
...
export const CreateStoryModal: React.FC<{ currentUser: User, songs: Song[], onClose: () => void, onCreate: (story: Partial<Story>) => void }> = ({ currentUser, songs, onClose, onCreate }) => {
    ...
    const handleCreate = () => {
        ...
        if (mode === 'image' && !imagePreview) { fileInputRef.current?.click(); return; }
        ...
        onCreate({
            ...
            image: mode === 'image' && imagePreview ? imagePreview : undefined,
            music: selectedMusic ? { ...selectedMusic } : undefined,
            createdAt: Date.now(),
            user: currentUser
        });
    };
                        ...
                        <div className="flex flex-col gap-2">
                            {songs.map(song => (
                                <div 
                                    key={song.id} 
                                    onClick={() => {
                                        setSelectedMusic({ url: song.audioUrl, title: song.title, artist: song.artist, cover: song.cover });
                                        setShowMusicPicker(false);
                                    }}
                                ...
                                >
                                    <img src={song.cover} className="w-14 h-14 rounded-lg object-cover shadow-md" alt="" />
